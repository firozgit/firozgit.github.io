...
- **post 5** — the cache as a data structure: preallocation, paging, prefix caching
- **post 6** — the scheduler: continuous batching and chunked prefill
- **post 7** — same answer, less work: online softmax, FlashAttention tiling, speculative decoding
- **post 8** — trading accuracy for memory: quantization, GQA, sliding window
- **post 9** — how do you know you didn't break it: perplexity and evaluation

The first four are about making inference correct. The rest are about making it
cheap, and then about how you tell whether cheap cost you anything.
...
Next: prompts of different lengths. That means padding, a mask that knows which
positions are real, and position ids that skip the pads.

::: {.article-navigation role="navigation" aria-label="Article navigation"}
[Next: Batching prompts of different lengths &rarr;](../02-padding/){.article-navigation-link .next}
:::
